...
class UartBusInterface:
    ...
    def connect(self):
        ...
        assert self.uart.isOpen(), f'failed to open device {self.interface} with boudrate {self.baud}'

        # Check status register
        #alue = self.read32(self.STS_ADDR)
        #f ((value & 0xFFFF0000) != 0xcafe0000):
        #   raise Exception("Target not responding correctly, check interface / baud rate...")

    ##################################################################
    # read32: Read a word from a specified address
    ##################################################################
    def read32(self, fpga_index, addr):
        # Connect if required
        if self.uart == None:
            self.connect()
        ...
        #                (addr >> 0) & 0xFF])
        cmd = [
                (self.CMD_READ & 0x7) | ((fpga_index & 0xf) << 3),
                (addr >> 0) & 0xFF,
                (addr >> 8) & 0xFF
              ]
        cmd = bytearray(cmd)
        ...
        # header
        b = self.uart.read(1)
        assert (int.from_bytes(b) & 0x7) == 3, f'bad ack header tag: {b}'
        print(f'got read ack header: {b}')
    ...
    ##################################################################
    def write32(self, fpga_index, addr, value):
        # Connect if required
        if self.uart == None:
            self.connect()
        ...
        #                (value >> 24) & 0xFF])
        cmd = [
                (self.CMD_WRITE & 0x7) | ((fpga_index & 0xf) << 3),
                (addr >> 0) & 0xFF,
                (addr >> 8) & 0xFF,
                (value >> 0) & 0xFF,
                (value >> 8) & 0xFF,
